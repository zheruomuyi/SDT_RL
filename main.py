...
def adjustJob():
    ...
    for key in keys:
        ...
        for asset_point in last_points:
            ...
            comp_error = last_point_json['compError'].double()
            comp_dev = last_point_json['compDev'].double()
            before_comp = last_point_json['beforeComp']
            after_comp = last_point_json['afterComp']
            comp_std = math.sqrt(comp_error / before_comp)
            comp_proportion = before_comp.long() / after_comp.long()
            if asset_point in environments.keys():
                env = environments[asset_point]
                s = env.render()
                change = GLOBAL_PPO.choose_action(s)
                action = {'change': change, 'comp_proportion': comp_proportion, 'comp_std': comp_std}
                env.step(action)
            else:
                env = Adjust_env(comp_dev, comp_std, comp_proportion)
                environments[asset_point] = env
...
if __name__ == '__main__':
    ...
    scheduler.add_job(adjustJob, 'interval', seconds=30)
